...
import xyz.doikki.videocontroller.httpcompoent.HttpClientManager;
import xyz.doikki.videocontroller.thread.WorkerThreadManager;
...
public class VideoInfoParserManager {
    ...
    public void parseVideoInfo(final String url, final IVideoInfoCallback videoInfoCallback) {
        if (videoInfoCallback == null || TextUtils.isEmpty(url)) {
            return;
        }
        WorkerThreadManager.submitRunnableTask(new Runnable() {
            @Override
            public void run() {
                ...
                try {
                    ...
                    if (response != null) {
                        ...
                        if (VideoType.isM3U8(contentType)) {
                            ...
                            VideoInfoParserManager.this.parseM3U8Info(url, response, videoInfoCallback);
                        }
                    }
                } catch (Exception e) {
